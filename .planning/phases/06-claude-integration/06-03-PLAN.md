...
<task type="auto">
  ...
  <action>
    ...
    3. Update handleAskSubmit function:
       - Check if on manager detail page: location.pathname starts with '/manager/'
       - Extract managerId from pathname if on detail page (split by '/' and get last segment)
       - If !apiKey:
         - Show alert: "Please enter your Anthropic API key first. Go to any manager page to configure."
         - Return early
       - If on Dashboard ('/'):
         - Navigate to first manager page: navigate('/manager/MGR001')
         - Note: The question will be sent after navigation via a useEffect in ManagerDetail, OR
         - Store the pending question in chatStore and send from ManagerDetail on mount
         - For MVP simplicity: Just navigate and show message "Select a manager to start chatting"
         - Return early without sending (chat context requires a specific manager)
       - If on ManagerDetail (location.pathname starts with '/manager/'):
         - The useChat hook in ManagerDetail handles the API call
         - Call the shared sendMessage via a ref or state pattern
         - For MVP: useChatStore actions are used directly here
         - Call addUserMessage(question)
         - Create client via createClaudeClient(apiKey)
         - Build context inline (manager overview + managerId-specific summary if available)
         - Start streaming, call API, handle events
         - On complete, call completeStreaming with extracted suggestions
         - Wrap in try/catch with setError

    4. Note: AppLayout and ManagerDetail share useChatStore, so messages appear in ChatThread automatically.
  </action>
  <verify>
