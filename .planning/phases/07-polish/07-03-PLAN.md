...
export function MessageActions({ content, messageId }) {
  ...
  const handleAddTo1on1 = () => {
    ...
    openConfirmationModal({
      type: 'add_to_1on1',
      manager: mentionedManager,
      topic: `Discuss: ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}`,
    });
  };
...
This ensures all interactive elements in the chat have visible focus indicators for keyboard navigation.
  ...
  <verify>Tab through chat interface, observe visible focus rings on input and buttons</verify>
